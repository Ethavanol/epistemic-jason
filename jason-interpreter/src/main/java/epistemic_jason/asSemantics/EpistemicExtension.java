...
import epistemic_jason.asSemantics.DEL.DELEvent;
...
import epistemic_jason.asSyntax.ASEpistemicSyntax;
import epistemic_jason.asSyntax.BaseLiteral;
...
import epistemic_jason.reasoner.EpistemicReasoner;
import epistemic_jason.reasoner.ReasonerType;
import epistemic_jason.reasoner.ReasonerTypeEnum;
import epistemic_jason.reasoner.SeparateAgentWorlds;
...
public class EpistemicExtension implements CircumstanceListener {
    ...
    private TransitionSystemEpistemic ts;
    private EpistemicReasoner reasoner;
    private boolean modelCreated;

    private final EpistemicParser parser = new EpistemicParser();

    //Map of listeners foreach agent
    private final List<ModelChangedListener> listeners = new ArrayList<>();

    public EpistemicExtension(TransitionSystemEpistemic ts) throws JasonException {
        this.ts = ts;
    ...
    public void modelCreateSem(String agentName) throws JasonException {
        // Do not re-invoke
        ...
        //add that the model changed for the lsiteners
        this.addModelChangedListener(this.ts.getAg());
    }
     ...
     * @return
     */
    protected List<Formula> getModelCreationConstraints() throws JasonException {
            ...
            this.ts.getAg().getBB().forEach(l -> {
                if (!l.isRule() && l.getNS() == Literal.DefaultNS) {
                    ...
                    if (!l.hasSource(new Atom("percept"))) {
                        // groundingBase.add(l);
                        EpistemicFormula newl = parser.parseFormulaToEpistemicFormula(l, false);
                        constraints.add(newl.toPropFormula(null));
                    }
                }
            });
    ...
    public ModelResponse getWorldsModelSem(String agentName) {
        if (!modelCreated) return null;
        return reasoner.getWorldsModel(separateAgentsModels.getSeparateAgentWorlds(), agentName);
    }
    /**
    ...
    public void applyEventModel(Event event, String agentName) throws JasonException {
        ...
        notifyListenersAppliedEvent(event);
    }


    public void replacePropsInModel(List<Pair<World, List<PropFormula>>> props, String agentName) throws JasonException {
        long startTime = System.nanoTime();

        reasoner.replacePropsInModel(props, separateAgentsModels.getSeparateAgentWorlds(), agentName);

        long reasonerEndTime = System.nanoTime();
        System.out.println("Total add Props time (ms): " + (reasonerEndTime - startTime) / 1000000);
    }


    private void addOnticLits(DELEventModel eventModel) throws JasonException {
        ...
        if (reasonerType.getReasonerType() == ReasonerTypeEnum.DEL && eventModel != null) {
            for (var e : eventModel.getDelEvents()) {
                for (var propForm : e.getPostCondition().keySet()) {
                    ...
                    if (!e.getPostCondition().get(propForm).equals(new PropFormula(new Pred(Literal.LFalse)))) {
    ...
    private DELEventModel createEventModel(OnEvent onEvent) throws JasonException {
        var relPlans = ts.getAg().relevantPlans(onEvent.getTrigger(), null);
    ...
    public List<Option> applicableOnPlans(List<Option> rp) {
        ...
        synchronized (ts.getC().syncApPlanSense) {
            ...
            for (Option opt : rp) {
                ...
                EpistemicFormula newContext = parser.parseFormulaToEpistemicFormula(context, false);
                ...
                if (context == null) { // context is true
                    ...
                    Iterator<RewriteUnifier> r = rewriteCons(newContext, opt.getUnifier());
                    ...
                    if (r != null) {
                        while (r.hasNext()) {
                            ...
                            if (!distinctUnifs.contains(simplifiedUnif)) {
                                ...
                                if(!context.isGround()){
                                    cPlan.setContext((LogicalFormula) simplifiedUnif.getFormula().simplify());
                                }
     ...
     */
    private Iterator<RewriteUnifier> rewriteCons(EpistemicFormula context, Unifier unifier) {
    ...
    private List<Formula> getRangeConstraintRules(AgentEpistemic ag, List<Literal> allRange) {
        // allRange should contain +/~ lits
        List<Formula> constraintRulesProps = new ArrayList<>();

        Map<Literal, Collection<EpistemicFormula>> headToBodyMap = new HashMap<>();
        ...
        for (Literal rangeLit : allRange) {
            ...
            for (Rule conRule : conRulesSet) {
                conRuleCount++;
                EpistemicFormula body = parser.parseFormulaToEpistemicFormula(conRule.getBody(), false);

                var rewriteIter = body.rewriteConsequences(ag, new Unifier());
        ...
        for (var entry : headToBodyMap.entrySet()) {
            var head = parser.parseFormulaToEpistemicFormula(entry.getKey(), false);
            ...
            for (var form : fullForm)
                groundDisjForms.add(form.simplify().toPropFormula(null));

            // Not sure if this is the right approach...
            constraintRulesProps.add(new ImpliesFormula(new OrFormula(groundDisjForms), head.toPropFormula(null)));
        }
    ...
    private Formula simplifyAndProp(LogicalFormula unpared) {
        EpistemicFormula newUnpared = parser.parseFormulaToEpistemicFormula(unpared, false);
        return newUnpared.simplify().toPropFormula(null);
    }
     ...
     * @return
     */
    private Collection<Rule> getCandidateRules(AgentEpistemic a, BeliefBase beliefBase, Literal literal, Unifier unif) {
    ...
    private Collection<Formula> getRangeConstraints(@NotNull AgentEpistemic ag, List<Literal> rangeOut) throws JasonException {
        BaseLiteral rangeVar = ASEpistemicSyntax.createBaseLiteral("range", ASSyntax.createVar());
     ...
     */
    private Formula propRange(Literal rangeLit, Unifier u, List<Literal> rangeValOut, BeliefBase belBase) throws JasonException {
        ...
        Literal litImp = ((Literal) firstTerm).forceFullLiteralImpl();

        BaseLiteral fullFirst = new BaseLiteral(litImp);
        // Add literal pos and neg
        BaseLiteral neg = new BaseLiteral(((Literal) fullFirst.clone()).setNegated(Literal.LNeg));
        BaseLiteral pos = new BaseLiteral(((Literal) fullFirst.clone()).setNegated(Literal.LPos));
        ...
        return createDisjunction(Set.of(pos.toPropFormula(null), neg.toPropFormula(null)));
    }
    ...
    public boolean evaluate(Literal litCons, Formula propFormula, String agentName) {
        ...
        return reasoner.evaluateFormula(propFormula, separateAgentsModels.getSeparateAgentWorlds(), agentName);
    ...
    public void replacePropsInModel(List<Pair<World, List<PropFormula>>> props){
        try {
            replacePropsInModel(props, ts.getAgArch().getAgName());
        } catch (JasonException ex) {
            throw new RuntimeException(ex);
        }
    }
    ...
    public void addModelChangedListener(ModelChangedListener modelChangedListener) {
        ...
        notifyListenersModelCreated();
    }

    public void removeApplyEventModelListener(String name) {
        listeners.remove(name);
    }

    private synchronized void notifyListenersModelCreated() {
        this.ts.getAg().modelCreated();
    }

    private synchronized void notifyListenersAppliedEvent(Event event) {
        this.ts.getAg().eventModelApplied(event);
    }
}
